...
SimData *read_sim_file(const char *filename) {
    ...
    /* ── Create shared memory for node semaphores ── */
    const char *shm_name = "/os_project_shm";
    ...
    /* Initialize one semaphore per node (value=1 = unlocked) */
    for (int i = 0; i < N; i++) {
        sem_init(&shm->node_sem[i], 1, 1); /* pshared=1 for processes */
    }
